...
No mirroring, no second player: the genuine hardware-decoding VLC window is reshaped via Win32, so there is zero added latency and every VLC feature and shortcut keeps working inside the PiP. A ~160KB dependency-free Rust daemon does the work; a tiny Lua extension adds the menu entry. See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) for how it works.

## Install

Download the [latest release](https://github.com/Bug-Finderr/vlc-pip/releases) zip, or build from source (needs the Rust MSVC toolchain):
...
- VLC 3.x only (3.0.23 verified). VLC 4.0 changes the video window architecture and needs re-validation.
