...
- **Fullscreen-origin PiP (v2.1.1).** Entering PiP from a fullscreen VLC is the same instant reshape as any enter - VLC's internal fullscreen state simply stays on for the whole PiP session (clearing it first cost the user ~1s of dead screen, and Qt only restores its windowed geometry from an untouched fullscreen window). The daemon hides VLC's fullscreen controller strip each tick and the keyboard hook swallows Esc alongside F, so nothing can trip Qt's state underneath the PiP. Exit restores the saved fullscreen style and rect verbatim: the user came from fullscreen and gets fullscreen back, internally consistent ([SPEC.md](SPEC.md) §7).
