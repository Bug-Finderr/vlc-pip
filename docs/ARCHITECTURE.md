...
- **Hooks follow the owned PiP session.** The idle daemon has no LL hooks. Each null slot installs independently while a session is active; failed installs retry without duplicating a live hook. Queued drag work reloads the owned state first; a changed snapshot or transition-lock contention clears the old gesture. Failed hook removals retry, with the cleared cache making any retained hook pass input through.
- **State/window transitions are process-serialized.** A named `VlcPipTransition` mutex covers each daemon pump action and each one-shot mutation. The daemon waits only 25 ms before returning to its message loop, preserving hook delivery and heartbeat updates during contention. One-shot convergence sleeps outside the mutex and locks only around each measurement/mutation.
