...
- **State/window transitions are process-serialized.** A named `VlcPipTransition` mutex covers each daemon pump action and each one-shot mutation. One-shot convergence sleeps outside the mutex and locks only around each measurement/mutation, keeping the hook pump responsive while preventing old restore and new enter operations from landing out of order.
