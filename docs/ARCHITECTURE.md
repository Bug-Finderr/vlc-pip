...
- **Fullscreen handoff (v2.1.1).** Entering PiP from a fullscreen VLC first makes VLC itself leave fullscreen (Esc posted to the vout child - its win32 window proc feeds VLC's hotkey engine, no focus needed); external reshaping alone would leave VLC's internal fullscreen state and its fullscreen-controller strip on screen, and save the fullscreen rect as the restore state. The daemon defers the enter across timer ticks (the pump must never block, see hooks above) until the caption is back for two consecutive ticks ([SPEC.md](SPEC.md) §7).
...
powershell -ExecutionPolicy Bypass -File scripts\smoke-test.ps1   # 38 end-to-end checks against live VLC
```
