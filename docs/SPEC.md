...
5. Named mutex `"VlcPipTransition"` serializes each mutating pump action with one-shot Enter/Exit/Restore. One-shot convergence sleeps outside it and locks only around each measurement/mutation.
6. Pump: `WM_HOTKEY` → Toggle + immediate session sync. `WM_TIMER` → consume request (`toggle`/`enter`/`exit` act; `stop` → `PostQuitMessage(0)`), load state + sync, maintain the fullscreen controller veil while a fullscreen-origin PiP is active, maintain_region, conditionally reload + sync if maintenance dropped state, then beat if >3 s. A normal sync from a cached session to no owned state reapplies the cached restore snapshot only when the state file is confirmed absent, covering a concurrent one-shot Exit after the tick's state load without treating a transient read/parse failure as exit; known terminal dissolve/heal syncs and daemon shutdown skip that repair. Transient file-I/O errors are swallowed (retry next tick); anything else propagates to the crash handler. The daemon creates no windows and accepts no text input; it handles its `WM_HOTKEY`/`WM_TIMER`/`WM_APP` thread messages directly, so `TranslateMessage` and `DispatchMessageW` are intentionally absent.
