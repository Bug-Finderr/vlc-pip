...
**The acceptance gate is language-agnostic:** `scripts/smoke-test.ps1` (40 checks: the 21 v1-era checks unchanged, plus 13 v2.1 gesture/heal checks, plus 6 v2.1.1 fullscreen-handoff checks), `scripts/uninstall.ps1`, and `extension/pip.lua` (descriptor version aside) carry over from v1. Only `pip-helper.exe`'s implementation swaps; every file format and observable behavior below must hold byte-for-byte.
...
- **Leave**: post `Esc` (`WM_KEYDOWN`+`WM_KEYUP`, scan 0x01) to **every** vout window of the VLC process - class prefix `"VLC video main"`, children AND top-levels, visible or NOT - plus the Qt top-level as a last resort. Fullscreen VLC hosts the vout either embedded (visible full-size child) or desktop-parented (an INVISIBLE top-level still at its stale windowed rect); both observed live on 3.0.23, and the visible-child-only lookup misses the second arrangement. The vout event thread processes posted keys regardless of focus and visibility (~20ms observed); Qt drops posted keys when unfocused (gotcha #7), so the top-level must never be the only target. Esc is leave-fullscreen only - a no-op otherwise - and both paths re-post while fullscreen persists, capped at 3 tries so a modal dialog is not Esc-spammed. Esc goes out only after the keyboard has been **quiet for two consecutive ticks** - no modifier key physically down (`GetAsyncKeyState`). One modifier-free instant is not enough: VLC translates posted keys against the physical modifier state at processing time, so the still-held Ctrl+Alt of the triggering hotkey turns Esc into Ctrl+Alt+Esc, bound to nothing (verified live, even unfocused) - and hotkey spam has inter-press gaps wide enough for a single tick to slip the Esc out mid-burst. The daemon refreshes the 2s give-up countdown until the chord is released - the user's own fingers control the wait.
- **Cloak**: the window is made invisible for the whole transition - `WS_EX_LAYERED` + alpha 0 the moment the handoff arms, removed again inside enter() right before its reshape (after the state save, so the check below stays race-free) - because the intermediate windowed restore is otherwise on screen for ~140ms (measured). `SW_HIDE` cannot do this: Qt re-shows the window as part of leave-fullscreen (show, style, and rect land in one shot ~25ms after Esc, verified live); transparency makes whatever Qt shows render as nothing. Alpha 0 verified to blank the D3D11 vout, fullscreen and windowed, with rendering intact after removal. The pre-cloak opacity state rides on a window property (cross-process, so a racing one-shot and the daemon agree): VLC's own "Windows opacity" setting uses `WS_EX_LAYERED`, so a user's layered bit and alpha are restored on uncloak and kept in enter()'s EXSTYLE snapshot - only a cloak-owned bit is masked out (a cloak must never become the restore state). uncloak is a no-op on a window that is not cloaked. Every drop path uncloaks owner-guarded (never touch a recycled handle): deadline/hard-cap give-up, stolen-by-one-shot (whose PiP on this very window must become visible), `exit`/`stop` requests, toggle-exit, and a failed enter(). Cloaking fails silently on an elevated VLC (UIPI): the transition is merely visible then. Accepted edge: a daemon force-killed mid-handoff (a sub-second window) strands an invisible VLC until its next own show.
- **Daemon path (deferred)**: the pump must never block (LL-hook timeout, §3). Toggle/enter on a fullscreen (or restored-but-still-captionless) VLC arms a pending enter keyed by hwnd + owner PID; Esc goes out once the window is observably fullscreen and the keyboard quiet (re-posts 2 quiet ticks apart). Each 150ms tick then requires the caption present on TWO consecutive ticks (Qt's style AND rect restores both landed) AND a quiet keyboard before calling enter - windowed ticks with a modifier down reset the count, so nothing can appear mid-spam only to be toggled away by the next press (observed live). The refreshable 2s deadline is bounded by an absolute **6s hard cap** from arm time: `GetAsyncKeyState` reads global keyboard state, so a stuck modifier or typing in another app must not hold the cloaked (invisible) VLC hostage - past the cap a windowed VLC enters despite held modifiers and a still-fullscreen one uncloaks and drops. The heal (§12) is paused while a pending is armed: its SetWindowPos on a still-fullscreen window would create a state that is neither fullscreen nor windowed and dead-stall the handoff. Dropped without entering when: the owner PID no longer matches (VLC died / handle recycled), something else entered PiP first (a one-shot CLI can win the race), the 2s deadline or 6s hard cap passes (Esc eaten, e.g. by a modal), or an `exit` or `stop` request arrives (`stop` must clear it - `PostQuitMessage` is not preemptive and the same tick still runs the pending check; uninstall would otherwise strand a PiP'd VLC with no daemon). Repeat toggles and enters while a pending is armed are NO-OPS: the handoff is invisible for its first ~0.5-1s, so an impatient second press must not cancel it (cancelling made spam ping-pong between arm and cancel - an even number of presses never entered PiP). A toggle with PiP already entered always exits, even while a pending is armed.
- **One-shot path (blocking)**: `toggle`/`enter` CLI modes do the same restore-then-judge (cloak and Esc quiet-gate included), then poll for the caption up to 2s (owner-PID guarded) and settle 100ms; timeout or owner change → exit 1 without entering (never save the fullscreen rect as restore state), uncloaking on every bail.
...
- **Test:** `cargo test` in `helper/` (pure logic: state JSON, geometry, options, request), then `scripts/smoke-test.ps1` (40 end-to-end checks against live VLC; requires install first and VLC closed).
...
Automated: `cargo test` green, then `scripts/smoke-test.ps1` → 40/40 PASS (enter/exit geometry + styles, topmost, minimal-look region, double/triple/spam-click immunity, hotkey + request-file interleaving without desync, exact rect restore, drag-move, edge drag-resize with the minimal look held, config.txt persistence + re-enter at persisted size, band-click/wheel no-ops, fullscreen-handoff enter/exit + repeat-toggle no-op + cloaked transition, close-in-PiP reopen heal).
