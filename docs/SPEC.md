...
```
Types: `hwnd`/`style`/`ex_style` pointer-sized signed integers (isize in memory, SPEC R4); `x y w h target_w target_h margin` i32 (`target_w`/`target_h` must be positive - anything else reads as no-state); `corner` one of `br bl tr tl` (unknown reads as `br`); `min` `1|0`; `pid` u32. `x..ex_style` are the pre-PiP restore data; `target_w..min` are the options in effect at Enter (so daemon and one-shot CLI converge on the same geometry); `pid` is the owner process.
