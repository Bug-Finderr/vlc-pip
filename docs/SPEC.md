...
- [ ] Menu toggle on a playing video shapes the box to the video's aspect (configured width kept; a scope film shows no bars); hotkey and CLI enters keep the configured box; the first drag on an adapted box never snaps its size.
...
- `maintain_region` is skipped while a drag is active; drag ticks and release clear only its stability debounce and adopt each applied rect as the vout-death watch baseline (§7), preserving the heal cadence fields. A release that changes the persisted state (resize, corner change) re-scopes the session at the next sync, dropping that baseline until the next live-child tick - an accepted sub-tick gap. After a resize, convergence re-clips with at most a ±2px correction.
