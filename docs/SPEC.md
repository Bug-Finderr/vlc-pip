...
### 5.1b `pip-intf.lua` (VLC Lua interface companion)
- Extensions receive listener events only while manually activated per session (VLC has no auto-activation), so continuous adaptation needs a Lua INTERFACE: VLC starts it automatically via `vlcrc` `extraintf=luaintf` (colon-separated list) + `lua-intf=pip`, and it runs its own loop thread.
- The loop publishes the playing video's size to `%TEMP%\vlc-pip-media.txt` (§6.6) every ~300 ms (`vlc.misc.mwait`), probing exactly like `trigger()` (English `Video resolution` key, `Orientation` transpose; the probe is duplicated because VLC gives intf and extension scripts no shared-module path). On a clean quit it removes the file.
- The daemon uses the published dims for hotkey/CLI/bare-request enters and to retarget a live PiP when the playlist moves to a differently shaped video (§7 enter, daemon loop step 6).
- Installs to `%APPDATA%\vlc\lua\intf\pip.lua`; a `vlcrc` already running a foreign `lua-intf` is never displaced (install warns and skips - the menu toggle still adapts).
...
- `restore` - install/uninstall maintenance restore. An owned state is restored and deleted; an unowned state is preserved for reopen heal. Exit 0 restored, 1 failed, 4 pending (2 remains the global unknown-mode code).
...
%APPDATA%\vlc\lua\intf\pip.lua                               (intf companion, from extension\pip-intf.lua)
%APPDATA%\vlc\pip\pip-helper.exe                             (helper, OUT of extensions)
%APPDATA%\vlc\vlcrc                                          (edited: extraintf gains luaintf, lua-intf=pip)
shell:startup\VLC PiP Daemon.lnk  ->  pip-helper.exe daemon  (login auto-start, no flash)
```
The `vlcrc` edits are surgical first-match line replacements written back UTF-8 without BOM, merging into an existing colon-separated `extraintf` list; a missing `vlcrc` gets a minimal one (VLC accepts it). Uninstall reverts to the stock commented defaults and removes only `luaintf` from the list.
...
First whitespace token, trimmed on read: `toggle` | `enter` | `exit` | `stop` (case-sensitive). `toggle` and `enter` accept an optional second token `v=WxH` - the extension's playing-video dimensions, used by enter's box adaptation; a malformed token or dimensions outside 1..=16384 read as absent, and further tokens are ignored. Consumed (read + delete) every 150 ms tick; read errors leave the file for the next tick; empty file is deleted and ignored. Extension and helper ship together (install.ps1): a newer extension against an older helper writes a token line the old exact-match dispatcher silently drops, leaving the menu inert until the helper is updated too. On daemon start, a pre-existing request is discarded **only if its command token is `stop`** (same tokenization as the pump) (a `pip-helper stop` with no daemon alive leaves one that would kill the fresh daemon on its first tick; a queued user toggle survives).
...
The smoke consumer deletes the previous file before launching `status`, waits for its exit, and requires a newly created file; status deliberately exits 0 even if its best-effort output channels fail.
...
### 6.6 `vlc-pip-media.txt` - the intf companion's now-playing dimensions
One line, no newline, rewritten every ~300 ms by `pip-intf.lua` (§5.1b):
```
<epoch-seconds> v=WxH     (a sized video is playing)
<epoch-seconds> -         (nothing playing, or no sized video / localized info keys)
```
Consumers accept the `v=WxH` only when `|now - epoch| < 5 s` (the publisher stops with VLC, and a force-killed VLC cannot delete the file - the same freshness rule as the daemon heartbeat, absolute difference for clock skew). The `v=` token parses exactly like the request channel's (§6.2, dims bounded 1..=16384 - two instances' interleaved buffered writes can splice a clean-parsing hybrid like `640x48060`); anything else reads as absent and the enter keeps the configured box. A clean VLC quit removes the file (VLC 3's lua `misc` has no `should_die`; shutdown surfaces as `mwait`'s "Interrupted." error, which the script catches to delete before exiting). Accepted edges: with two instances publishing, the last writer wins; the ~300 ms poll lags item changes, so an enter inside that lag can briefly adapt to the previous item before the next publication corrects it (the daemon settles an enter's own dims to narrow this).
...
4. With media dimensions present (precedence: the request's explicit `v=WxH` - probed at the menu click - then the intf companion's fresh publication (§6.6), which covers hotkey, CLI, and bare-request enters; the visible raster size either way - VLC exposes no SAR-corrected size, so anamorphic media adapts to its raster shape), adapt the box: keep the configured width, derive the height from the video's aspect (floor 1), and shrink at that aspect when the height would exceed 80% of the work area minus the measured chrome height - the same envelope as resize's cap, so the first drag on an adapted box never clamps it - with the 256px width floor winning, matching resize. Degenerate media dims keep the configured box. The adapted box is the target everywhere below (state `target_w/h`, corner, chrome landing).
...
Entering PiP from a fullscreen VLC is the same immediate reshape as any other enter - the PiP appears at the keypress. **VLC's internal fullscreen state stays ON for the whole PiP session.** Clearing it first (post Esc, wait for Qt's windowed restore) cost the user ~0.5-1s of dead screen; the reverse order desyncs Qt, which restores its windowed geometry only from an UNTOUCHED fullscreen window - after an external reshape, Esc left a captionless window at the PiP rect with the menus grown back (verified live). A fullscreen-origin PiP is recognized when its saved pre-PiP style lacks the complete two-bit `WS_CAPTION` mask (an iconic VLC is restored before the snapshot, as always).
...
- **Dissolve on media end / stop**: VLC leaves fullscreen internally BY ITSELF when playback ends or is stopped - no input involved - and its re-layout balloons the window to Qt's idea of windowed geometry within ~a tick of the vout dying (verified live). The vout-death watch (§7 maintain_region step 2) fires immediately for fullscreen-origin sessions (vout gone AND the rect moved off the watch baseline - the last rect seen with live video, an enter's seeded landing rect, or our own gesture's applied rect; runs regardless of `min`): the PiP session then dissolves - frame styles back at Qt's chosen rect, controller veil cleared, state deleted. Stock VLC lands windowed after fullscreen playback ends too, and the saved fullscreen rect must never be restored onto an internally windowed VLC. A toggle after the dissolve is a fresh windowed-origin enter.
...
Cross-tick state: the previous (window, child) rects held as an Option (None = reset; reset on missing/stale state, no child, and after our own resize), the vout-death watch's baseline (last window rect seen WITH a live child, seeded with the landing rect of a daemon or one-shot enter, or applied by our own gesture), the last media dims the session settled its box to (daemon retarget, loop step 6), plus the heal retry counter and bounded absent-process snapshot wait. The daemon resets the baselines and settled media whenever the session IDENTITY changes - the cached (hwnd, pid), where a vacant session is (0, 0) - in the same sync that resets input state, so a fresh PiP never inherits a stale baseline. The daemon's own parameter rewrites (drag release, media retarget) keep the same identity and therefore keep the baselines, the settled media, and armed gestures: treating them as session changes wiped the vout-death baseline exactly during playlist vout gaps and re-fired the retarget against argv options after every resize drag. The heal fields are not session-scoped. A drag clears only the stability debounce, and each gesture apply adopts its applied rect as the watch baseline - dragging a stopped PiP must never read as Qt taking the frame over.
The function reports a terminal dissolve/heal only after the state file is absent (`remove_file` success or NotFound). A removal failure preserves the dissolve baseline or heal terminal state and reports false so the next tick retries. On success, the daemon conditionally reloads state and re-syncs its cache/hooks in the same tick; ordinary ticks keep one shared state-file load, while the one-shot converger ignores the signal.
1. Load state; missing → reset, return. Stale → reset, hand to the reopen heal (§12), return. Read the window rect BEFORE probing for the child: a release between the two calls then pairs as (old rect, no child) - a one-tick delay - never as (balloon rect, live child), which would poison the baseline.
2. **Vout-death watch** - before the `min` gate, it guards every session; an iconic window is skipped entirely (the minimized shell rect is neither a baseline nor Qt taking over). The video child is the first visible child (recursive) whose class starts with `"VLC video main"`. A live child records the window rect as the watch baseline. No child AND a rect off the baseline = Qt released the video widget and took the frame over (media end, stop, and an audio-only playlist item all do this with no input: `qt-video-autoresize`, default on, restores the remembered pre-video interface size and drops topmost). A fullscreen origin dissolves immediately at Qt's chosen rect (§7 Fullscreen-origin PiP) - Qt left fullscreen internally and the fullscreen shell must never come back. A windowed origin HOLDS instead: `SetWindowPos(h, HWND_TOPMOST, baseline, SWP_NOACTIVATE)` snaps the frame back to the last live rect and reasserts the dropped topmost; the session survives, and the next vout re-clips into the same box. Three windowed exclusions: a re-captioned frame is an exit whose state delete is pending retry (never snap that back); Qt's takeover always resizes, so a SAME-SIZE move (Win+Shift+arrow, monitor changes) adopts the new rect as baseline instead of fighting it; a baseline whose monitor no longer exists adopts the OS's relocated rect rather than parking the box off-screen. With autoresize off the frame never moves and the watch stays idle.
3. `min=0` → return. No child → reset debounce, clear region if present, return - the stopped mini UI shows in the held box.
4. **Two-tick stability debounce**: read window + child rects; act only if both are UNCHANGED since the previous tick (VLC re-fits the child asynchronously after our resize; acting on unsettled rects caused perpetual resize thrash in v1). Always record current rects.
5. **Chrome sanity clamp**: chrome = window minus child size; if any dimension is negative or > 300 px → stale rects, return.
6. Nonpositive targets, invalid rect sizes, or unrepresentable checked arithmetic → return. Otherwise, when the child is not at target size (tolerance ±2 px), recompute the corner for the video, resize the window to `target + chrome` positioned so the CHILD lands at the corner (`SetWindowPos(h, HWND_TOPMOST, tx, ty, tw, th, SWP_FRAMECHANGED)` - no SWP_SHOWWINDOW here), drop the stored rects (our own resize), return.
7. Child at target: verify the region **box** against the child-relative rect (a live-clipped resize drag leaves an approximate region) and set it on mismatch: `CreateRectRgn` + `SetWindowRgn`; **on failure `DeleteObject` the region - the system owns it only on success**.
...
4. Under the transition mutex, load state and run the full owner-checked session/hook sync before the first heartbeat (a daemon restarted while already in PiP is guarded from the first message). Startup gives up after one second so the installer's five-second verification deadline remains meaningful.
5. Named mutex `"VlcPipTransition"` serializes each mutating pump action with one-shot Enter/Exit/Restore. The pump waits at most 25 ms; on contention it refreshes an overdue heartbeat, reposts hotkey/drag work, clears the old gesture before resuming, and lets timer work retry on the next tick. One-shot convergence sleeps outside the mutex and locks only around each measurement/mutation.
6. Pump: `WM_HOTKEY` → Toggle (with the intf companion's fresh dims, §6.6) + immediate session sync + watch-baseline seed. `WM_TIMER` → consume request (`toggle`/`enter`/`exit` act, enters falling back to fresh published dims when the request carries no `v=`; `stop` → `PostQuitMessage(0)`), load state + sync (seeding the baseline after a request enter), maintain the fullscreen controller veil while a fullscreen-origin PiP is active, **retarget to the playing video** (an owned `min` session, not mid-drag, with fresh published dims differing from the last settled ones: recompute the adapted box with the chrome measured now and save the new `target_w/h` - the converger executes the resize on following ticks; a transient save failure retries because the settled dims only update on success), maintain_region, conditionally reload + sync if maintenance dropped state, then beat if >3 s. `WM_APP` drag work reloads and syncs owned state before applying a queued gesture, so an external session transition clears stale work. Transient file-I/O errors are swallowed (retry next tick); anything else propagates to the crash handler. The daemon creates no windows and accepts no text input; it handles its `WM_HOTKEY`/`WM_TIMER`/`WM_APP` thread messages directly, so `TranslateMessage` and `DispatchMessageW` are intentionally absent.
7. Cleanup on loop exit: sync to no session (retrying each installed hook once), unregister the hotkey, delete the alive file, and clear heartbeat ownership. A final failed unhook remains until process exit.
...
- **Install/upgrade:** before mutation, the script validates the current state's trailing newline, exact 13-token envelope, unsigned 32-bit PID, and installed-helper presence. A malformed envelope or PID aborts. It then stops only daemon processes whose resolved path equals the installed executable, preventing a timer tick from racing restoration. The installed old helper's `restore` mode performs the full HWND+PID ownership check: owned state must restore and disappear, while an unowned record remains pending heal. Any other result restarts the verified old daemon and aborts. The v2.1.1 `vlc-pip.json` file is deleted as obsolete cleanup, never parsed or migrated. After copying the helper and Lua extension, startup succeeds only when the launched process still owns the installed path and writes a fresh PID-bound heartbeat with `hotkey=1 timer=1`.
- **Test:** run the fmt, unit, Clippy, release-build, PowerShell-contract, parser, and package gates in `.github/workflows/ci.yml`; then install with VLC closed and run `scripts/smoke-test.ps1` for the live gate.
- **Uninstall:** validate state, stop the exact-path daemon, then run the same maintenance restore before removing installed/runtime files. Refuse to uninstall while reopen heal is pending, or when state is corrupt or cannot be restored; never discard unresolved current state. A post-stop refusal restarts and verifies the old daemon so pending heal can continue.
...
- Adaptation degrades to the configured box wherever VLC's Lua cannot report dims: a localized VLC (English `Video resolution` key only - this disables BOTH the menu probe and the publisher), a `vlcrc` running a foreign `lua-intf` (install refuses to displace it; the menu toggle still adapts there), or a VLC started before the install (the intf loads on the next VLC start). Anamorphic media adapts to its raster shape (no SAR-corrected size in VLC 3's Lua), and with two VLC instances the media file is last-writer-wins.
- Explicit `w=`/`h=` daemon arguments win over gestures at every adaptation, as they always did over config at enters: an item-change retarget re-derives from the argument width, so a resize drag lasts only until the playing video's shape next changes. Config-driven sizing (the default) keeps drags fully, because a resize release persists its width as the new knob.
...
Automated: `cargo test` green, then `scripts/smoke-test.ps1` → ALL PASS. The live test verifies the heartbeat's freshness, PID, installed-executable identity, hotkey, and timer; enter/exit geometry, styles, exact topmost state, region, state deletion, and rect restore; separate 2/3/5-click guards; heartbeat, input guards, and deferred hotkey application during transition-lock contention; hotkey/request sequencing; drag, resize, wheel, and persistence; every matching fullscreen controller veiled during PiP and unveiled after exit; daemon-active one-shot exit races plus 50 serialized exit-enter cycles; install during fullscreen PiP; pending-heal uninstall refusal and daemon restart; fullscreen stop dissolve; close-in-PiP reopen heal; the windowed stop hold (box, topmost, session survival) with an exact restore on the following exit; and the intf companion's fresh media publication with a bare enter adapting to it (assumes screen:// reports the primary resolution). It also validates virtual-desktop bounds. Absolute input on a non-primary monitor is probed only when multiple monitors exist; this branch is configured but untested on the current one-monitor host.
...
- [ ] F / double-click work normally when NOT in PiP; F is swallowed only while VLC is focused in PiP, and Esc only in a focused fullscreen-origin PiP.
- [ ] Media end or stop in a windowed PiP keeps the box in place: Qt's balloon is snapped back within a tick with topmost intact, the mini UI shows in the box, and restarting playback (or the next playlist item, including across an audio-only track) re-clips into the same box.
- [ ] Dragging a stopped (childless) PiP moves it without a snap-back fight.
- [ ] Daemon survives VLC close and re-arms on next VLC launch; login shortcut starts it flash-free.
- [ ] Drag-resize works from all four corners (the smoke test can only reach the right edge); drags feel smooth; the nominal 80% cap holds, with the 256px minimum winning on smaller work areas.
- [ ] Menu toggle on a playing video shapes the box to the video's aspect (configured width kept; a scope film shows no bars); the first drag on an adapted box never snaps its size.
- [ ] With the intf companion loaded (VLC restarted once after install), hotkey and CLI enters adapt the same way, and a playlist advancing to a differently shaped video reshapes the live box within ~a second. Without it (localized VLC, foreign `lua-intf`, stale publisher) they keep the configured box.
- [ ] A move-only drag release updates the corner in `config.txt` but keeps the configured size (the adapted target must not leak in).
...
- `maintain_region` is skipped while a drag is active; drag ticks and release clear only its stability debounce and adopt each applied rect as the vout-death watch baseline (§7), preserving the heal cadence fields. The release's state rewrite keeps the session identity, so that baseline survives into the following ticks. After a resize, convergence re-clips with at most a ±2px correction.
