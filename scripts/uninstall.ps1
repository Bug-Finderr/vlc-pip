...
if ((Test-Path "$env:TEMP\vlc-pip.json") -and (Test-Path $exe)) { Start-Process $exe exit -Wait }
...
Remove-Item "$env:TEMP\vlc-pip.json", "$env:TEMP\vlc-pip-request.txt", "$env:TEMP\vlc-pip-daemon.alive", "$env:TEMP\vlc-pip-status.json", "$env:TEMP\vlc-pip-crash.txt" -Force
